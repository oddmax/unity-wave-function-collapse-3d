...
public class TilePainter : MonoBehaviour{
	...
	static GameObject CreatePrefab(UnityEngine.Object fab, Vector3 pos, Vector3 rot) {	
		GameObject o = PrefabUtility.InstantiatePrefab(fab as GameObject) as GameObject; 
		if (o == null){
			Debug.Log(IsAssetAFolder(fab));
			return o;}
		o.transform.position = pos;
		o.transform.localEulerAngles = rot;
		return o;
	}
	...
	public void Restore(){
		...
		for (int i = 0; i < palette.Count; i++){
			...
			if (IsAssetAFolder(o)){
				...
				if (o != null)
				{
					...
					rot.y = 0;
					GameObject g = CreatePrefab(o, new Vector3(), rot);
		...
		tileobs = new GameObject[width, depth];
		if (tiles == null){
	...
	void OnValidate(){
		...
		bounds.center = new Vector3((width*gridsize)*0.5f-gridsize*0.5f, 0f, (depth*gridsize)*0.5f-gridsize*0.5f);
		bounds.size = new Vector3(width*gridsize, 0f, (depth*gridsize));
	}
	...
	public bool ValidCoords(int x, int z){
		if (tileobs == null) {return false;}
		
		return (x >= 0 && z >= 0 && x < tileobs.GetLength(0) && z < tileobs.GetLength(1));
	}
	...
	public void OnDrawGizmos(){
		...
		Gizmos.DrawWireCube( new Vector3((width*gridsize)*0.5f-gridsize*0.5f, 0f, (depth*gridsize)*0.5f-gridsize*0.5f),
			new Vector3(width*gridsize, 0f, (depth*gridsize)));
	}
	#endif
}
 ...
 public class TileLayerEditor : Editor{
	...
	public override void OnInspectorGUI () {
		...
		GUILayout.Label("[q]+click  : sample tile color");
